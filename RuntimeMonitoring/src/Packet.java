...
public class Packet implements java.io.Serializable {
	...
	private Instant time = null;
	  ...
	  public Packet(MessageType type, Timestamp localTimestamp,Instant time,int indexFrom) {
		  if(type == MessageType.NORMAL_RECEIVE) {
			  this.type = type;
			  this.localTimestamp = new Timestamp(localTimestamp);
			  this.time = time;
			  this.indexFrom = indexFrom;
			   localStat = null;
			   allLocalEvents = null;
	  ...
	  public Packet(MessageType type, LocalTraceCollector allLocalEvents,int indexFrom,Instant time, Timestamp localTimestamp) {
		  if((type==MessageType.CONFIG_FINISH)||(type==MessageType.WINDOW_REPORT)) {
			  this.type = type;
