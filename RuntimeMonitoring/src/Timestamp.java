...
public class Timestamp implements java.io.Serializable {
	...
	public void setType(TimestampType tt) {
		this.type=tt;
	}
    ...
    public void timestampLocalEvent(Instant in) {
	    if(this.type == TimestampType.NO_TIMESTAMP) return;
	    timestampLocalEventHLC(in);
	    if(this.type == TimestampType.HLC) return;
		localCausalityClock.timestampLocalEvent();
	}
	...
	public void timestampReceiveEventHLC(Timestamp m, Instant currentTime) {//version where instant is provided
		if(type == TimestampType.NO_TIMESTAMP) return;
		Instant temp_l = l;
		Instant l_m = m.getL();
		long c_m = m.getC();
		l = SimpleMessageUtilities.maxInstant(temp_l, SimpleMessageUtilities.maxInstant(l_m ,currentTime));
	...
	private void timestampSendEventHLC(Instant currentTime) {//version where instant is provided
		if(type == TimestampType.NO_TIMESTAMP) return;
		Instant temp_l = l;
		l = SimpleMessageUtilities.maxInstant(temp_l, currentTime);
		if( l.equals(temp_l)) {
			c++;
		} else {
			c=0;
