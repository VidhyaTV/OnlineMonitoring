...
public class SimpleMessageUtilities {
	...
	public static Instant maxInstant(Instant a, Instant b) {
	 ...
	 public static void spinWaitMicros(long micros){
		...
		//System.out.println("After wait:"+Instant.now());
    }
	 public static void spinWaitMicrosNew(long micros){
			//System.out.println("Before wait:"+Instant.now());
			Instant waitUntil = Instant.now().plusNanos(micros * 1000);
			//System.out.println("waitUntil:"+waitUntil);
	        while(waitUntil.isAfter(Instant.now())) {
	        	//System.out.println("During wait:"+Instant.now()+" waiting for "+waitUntil);
	        }
	        //System.out.println("After wait:"+Instant.now());
	}
	...
	//credit: https://stackoverflow.com/a/6441483/2959347
		...
		try {	 
			...
			//System.out.println("cmd /c "+cmd);//for windows 
